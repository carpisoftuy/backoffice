...
<body>
    ...
    <div class="container-form">
        <form class="formulario-almacen" method="POST" action="../../almacenes/{{$almacen->id}}">
            <input type="hidden" name="_method" value="put" />
            <label for="espacio" name="espacio">Espacio</label>
            <input type="number" name="espacio" required value="{{$almacen->espacio}}">
            <label for="espacio_ocupado" name="espacio_ocupado">Espacio ocupado</label>
            <input type="number" name="espacio_ocupado" required value="{{$almacen->espacio}}">
            <label for="id_ubicacion" name="id_ubicacion">Ubicación</label>
            <select name="id_ubicacion">
                @foreach ($direcciones as $direccion)

                <option value="{{$direccion->id}}">{{$direccion->id}}: {{$direccion->direccion}}</option>
                @endforeach
            </select>
